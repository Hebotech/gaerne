...
async function fetchProducts(set) {
  try {
    ...
    let companiesArray = companies.filter((company)=>company.properties.gaerne && company.properties.website ?   company.properties.gaerne.value==='true' :false).map(({ properties: company }) => {
      
      return {
        name: company.name ? company.name.value : company.website.value || 's',
